...
**`notifications` — this is what the control panel UI calls "Banners" under a promotion's edit page (Marketing → Promotions → [promotion] → Banners → "Add banner"), confirmed live — do NOT confuse with the general storefront `/v2/banners` content-block API below, which is a completely different feature.** These are short on-brand text messages shown to shoppers in specific spots as a promotion is/isn't yet satisfied, not images. Each entry: `{"content": "...", "type": "UPSELL"|"ELIGIBLE"|"APPLIED", "locations": [...]}`, all three required. The control panel UI shows 4 template choices (Availability, Congratulations, Eligibility, Upsell) but only 3 map to the actual `type` enum (`APPLIED` = "Congratulations", `ELIGIBLE` = "Eligibility", `UPSELL` = "Upsell" — "Availability" doesn't correspond to any enum value found in this pass, so it may not be settable via this field at all). `locations` valid values (confirmed via a deliberate-error probe, since the spec doesn't enumerate them): `HOME_PAGE`, `PRODUCT_PAGE`, `CART_PAGE`, `CHECKOUT_PAGE` (accepts either case). `PUT /v3/promotions/{id}` with just `{"notifications": [...]}` is a **partial update** — confirmed live, it doesn't clobber `rules`/other fields, so you don't need to resend the whole promotion body just to add banners to an existing one.
